...
export default function ProductManagerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="glass-card w-full max-w-3xl rounded-2xl border border-slate-700 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/80">
          <div className="flex items-center gap-2">
            <Package className="w-5 h-5 text-emerald-400" />
            <h2 className="text-lg font-bold text-white">Product & Inventory Manager</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          {/* Create Product Form */}
          <form onSubmit={handleSubmitNew} className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 space-y-4">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Add New Product</h4>
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 transition shadow-md"
            >
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider">Current Stock Inventory</h4>
                ...
                <tbody className="divide-y divide-slate-800">
                  {products.map((prod) => (
                    <tr key={prod.id || prod._id} className="hover:bg-slate-800/40">
                        ...
                        {editingId === (prod.id || prod._id) ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              className="w-16 bg-slate-950 text-white border border-emerald-500 rounded px-1.5 py-0.5"
                            />
                            <button
                      ...
                      <td className="p-3 text-amber-400 font-mono">{prod.reserved_stock}</td>
